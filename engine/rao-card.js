/* ============================================================================
   ...
   THE LAWS this file enforces (each has a guard in tools/verify-calm.js):
     ...
     4. HELP ACCUMULATES — hint bubbles and walkthrough steps only ever stack;
        ...
        point so the panel stands alone. AMENDED by BRIEF-SHOWN-ANSWER-1
        (2026-07-24): the exception covers all THREE commit paths —
        openWalkthrough, revealSolution, AND showAnswer. The question DOM is
        still untouched; only the bubble stream (outside .qbody) is removed,
        so the no-repaint law holds.
...
function wireCard(frame) {
  ...
  function showAnswer() {
    ...
    quietChrome(true);
    if (hintBtn) hintBtn.style.display = "none";
    // BRIEF-SHOWN-ANSWER-1 (RULED by Venkat 2026-07-24): the shown-answer
    // commit clears the accumulated hint bubbles EXACTLY as openWalkthrough
    // and revealSolution do — Item 63's sanctioned exception now covers all
    // THREE commit paths. The chat lives OUTSIDE .qbody, so the no-repaint
    // law holds here the same way it does on the other two.
    if (chat) { chat.remove(); chat = null; }
    ...
    // BRIEF-SHOWN-ANSWER-1 (same ruling, closes #89): the old line "Here's the
    // answer — you've got this!" read as praise for failing twice. Replaced
    // verbatim per the ruling — do not restore the old line from a stale note.
    var html = '<p class="cc-shown-line">No worries, let\'s try a new one!</p>';
