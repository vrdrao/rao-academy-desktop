# BRIEF-FEEDBACK-PROTOCOL-2026-07-19 — create docs/FEEDBACK-PROTOCOL.md
...
1. HEAD is a descendant of 0e62cdb; working tree has no modified tracked
   files (untracked Grade-3 work files are expected and fine — list them
   in the report, touch none of them).
2. `GRADE-ROLLOUT-PLAYBOOK.md` exists at repo root (version playbook-2
   or playbook-3 — report which).
3. `docs/` exists.

## Task — write docs/FEEDBACK-PROTOCOL.md

This is chat-authored content; write it VERBATIM as given below. Your
only judgment call is mechanical markdown correctness. Do not add,
remove, or rephrase rules.
...
One commit containing exactly: `docs/FEEDBACK-PROTOCOL.md` and this
brief archived to `docs/briefs/` (md5-verify archive, remove root copy).
Commit message: `feedback-protocol-1: triage classes and batch cycle for
lesson review`.
Do NOT push.

## Report (paste back to chat)

1. Entry-check results (HEAD, playbook version found, untracked list).
2. md5 of the created docs/FEEDBACK-PROTOCOL.md.
